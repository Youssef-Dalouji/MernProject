...
const ChatOnline=() => {
    ...
    const queryExistRoom=useQuery('findRooms',()=>{
        return axios.post('http://localhost:80/masterServer/room/query',{idRoom:iRdecr},{headers:{Authorization:`Bearer ${window.sessionStorage.getItem('idUserSerie')}`}})
    },{
        refetchInterval:1000,
        refetchIntervalInBackground:true
    })
    const queryInformationsRoom=useQuery('GETInformationsRoom',()=>{
        return axios.post('http://localhost:80/masterServer/room/queryInformationsRoom',{idRoom:iRdecr},{headers:{Authorization:`Bearer ${window.sessionStorage.getItem('idUserSerie')}`}})
    },{
        refetchInterval:1000,
        refetchIntervalInBackground:true
    })
    const queryExistUserInRoom=useQuery('ExistUserInRoom',() => {
        return axios.post('http://localhost:80/masterServer/room/ConnectRoom',{idRoom:iRdecr},{headers:{Authorization:`Bearer ${window.sessionStorage.getItem('idUserSerie')}`}})
    },{
        refetchInterval:1000,
        refetchIntervalInBackground:true
    })

    const connectWithDashborad=useQuery('queryDashborad',() => {
        return axios.get('http://localhost:80/dashboradServer/dashborad-info-TC')
    },{
    ...
    const queryPingUser=useQuery('ExistUserOnLine',() => {
        return axios.post('http://localhost:5500/M-1351919175/queryPingConnect',{idRoom:iRdecr},{headers:{Authorization:`Bearer ${window.sessionStorage.getItem('idUserSerie')}`}})
    },{
        refetchInterval:1000,
        refetchIntervalInBackground:true
    })
    const getDataUserChat=useQuery('DataUserChat',() => {
        return axios.post('http://localhost:80/masterServer/M-1351919175/get',{idRoom:iRdecr},{headers:{Authorization:`Bearer ${window.sessionStorage.getItem('idUserSerie')}`}})
    },{
        refetchInterval:1000,
        refetchIntervalInBackground:true
    })
    const envMessage=() => {
      if(message!==''){
        ...
        const SendMsg=async ()=>{
            let resu=await axios.post('http://localhost:80/masterServer/M-1351919175/create',{message:message,idRoom:iRdecr,name:ndecr},{headers:{Authorization:`Bearer ${window.sessionStorage.getItem('idUserSerie')}`}})
